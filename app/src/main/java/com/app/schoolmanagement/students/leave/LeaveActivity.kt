...
class LeaveActivity : AppCompatActivity(), KodeinAware, LeaveListener,
    ...
    override fun onSuccess(data: Timetable) {
        ...
        if (data.response?.isPdf == "1") {
            ...
            pdfViewer.loadUrl("http://docs.google.com/gview?embedded=true&url=" + Constants.base_url+ Constants.teacher + data.response.pdfPath)
            pdfViewer.visibility = View.VISIBLE
        } else if (data.response?.isPdf == "0") {
            Picasso.get().load(Constants.base_url+ Constants.teacher + data.response.imgPath).fit().into(imageView)
